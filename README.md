...
- `Density/` — три ступени плотности, по словарю на каждую: `Compact`, `Normal` и
  `Comfortable`. Словарь подмешивается поверх темы и переопределяет только то, что от
  плотности зависит — высоты строки, контролов и хрома, шаг лестницы дерева, отступы
  внутрь контролов строки и всю шкалу расстояний; контур фокуса, скругления, цели
  попадания и размер иконки от плотности не зависят намеренно. Словарь обычной
  ступени пуст: её значения — те, что объявлены в `Metrics.axaml` и `Spacing.axaml`.
- `ControlThemes/` — темы всех Ax\*-контролов, меню и контекстных меню, тултипа,
  скроллбара ключом `{x:Type ScrollBar}` для всего приложения — дорожка 12 всегда,
  а меняется в ней бегунок: 6 в покое, 8 под курсором, — и кольцо фокуса
