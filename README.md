...
- `ControlThemes/` — темы всех Ax\*-контролов, меню и контекстных меню, тултипа,
  тонких скроллбаров (8px) ключом `{x:Type ScrollBar}` для всего приложения, и кольцо фокуса
  `AxFocusRing` — одна часть на все шаблоны: форма называется словом (`Outside`, `Row`, `Inside`),
  а из чего она сложена, знает её тема.
